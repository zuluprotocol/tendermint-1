...
func (cli *grpcClient) GetSnapshotChunkAsync(params types.RequestGetSnapshotChunk) *ReqRes {
	req := types.ToRequestGetSnapshotChunk(params)
	res, err := cli.client.GetSnapshotChunk(context.Background(), req.GetGetSnapshotChunk(), grpc.WaitForReady(true))
	if err != nil {
		cli.StopForError(err)
	}
	return cli.finishAsyncCall(req, &types.Response{Value: &types.Response_GetSnapshotChunk{GetSnapshotChunk: res}})
}
...
func (cli *grpcClient) GetSnapshotChunkSync(
	params types.RequestGetSnapshotChunk) (*types.ResponseGetSnapshotChunk, error) {
	reqres := cli.GetSnapshotChunkAsync(params)
	return reqres.Response.GetGetSnapshotChunk(), cli.Error()
}
